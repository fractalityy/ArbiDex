...
interface IMasterChefV2 {
    ...
    function pendingArbi(uint256 _pid, address _user) external view returns (uint256);
...
contract ArbiPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 arbiAtLastUserAction; // keep track of arbi deposited at the last user action.
    ...
    event NewVArbiContract(address VArbi);
    ...
    /**
     * @notice Constructor
     * @param _token: Arbi token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: arbi pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(arbiPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the arbi owner address or the operator address.
     */
    modifier onlyOperatorOrArbiOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or arbi owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onArbiPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.arbiAtLastUserAction;
    ...
    /**
     * @notice Unlock user arbi funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrArbiOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Arbi Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in ARBI)
    ...
    function depositOperation(
        ...
        if (VArbi != address(0)) {
            IVArbi(VArbi).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.arbiAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Arbi Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VArbi != address(0)) {
            IVArbi(VArbi).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.arbiAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.arbiAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending ARBI tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingArbi = masterchefV2.pendingArbi(arbiPoolPID, address(this));
        if (pendingArbi > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(arbiPoolPID, 0);
    ...
    /**
     * @notice Set VArbi Contract address
     * @dev Callable by the contract admin.
     */
    function setVArbiContract(address _VArbi) external onlyAdmin {
        require(_VArbi != address(0), "Cannot be zero address");
        VArbi = _VArbi;
        emit NewVArbiContract(VArbi);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Arbi Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingArbiRewards();
     ...
     * @return Returns total pending arbi rewards
     */
    function calculateTotalPendingArbiRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingArbi(arbiPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingArbiRewards()) * (1e18)) / totalShares);
    }
