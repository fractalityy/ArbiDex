...
contract IFOInitializable is IIFOV2, ReentrancyGuard, Ownable {
    ...
    function harvestPool(uint8 _pid) external override nonReentrant notContract {
        ...
        if (_poolInformation[_pid].totalAmountPool > _poolInformation[_pid].raisingAmountPool) {
            offeringTokenAmount = (_userInfo[msg.sender][_pid].amountPool * _poolInformation[_pid].offeringAmountPool) / _poolInformation[_pid].totalAmountPool;
        }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint8 i = 0; i < NUMBER_POOLS; i++) {
            ...
            if (_poolInformation[i].totalAmountPool > _poolInformation[i].raisingAmountPool) {
                claimablePools[i] = (_userInfo[_user][i].amountPool * _poolInformation[i].offeringAmountPool) / _poolInformation[i].totalAmountPool;
            }
        }
